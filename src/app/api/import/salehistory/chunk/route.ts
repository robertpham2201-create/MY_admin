...
export async function POST(request: Request) {
  try {
    ...
    const { items, provider } = (await request.json().catch(() => ({}))) as {
      items?: unknown;
      provider?: unknown;
    };

    if (!Array.isArray(items)) {
      return NextResponse.json({ ok: false, error: "bad_request", message: "Expected { items: [...] }" }, { status: 400 });
    }
    ...
    for (const it of items) {
      ...
      if (Array.isArray(it.Products)) {
        for (const p of it.Products) {
          ...
          const lineId = typeof prod.Id === "number" ? Math.trunc(prod.Id as number) : null;
          if (!lineId) continue;
          orderItems.push({
            line_id: lineId,
            order_id: orderId,
