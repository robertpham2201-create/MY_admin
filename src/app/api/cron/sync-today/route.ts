...
async function fetchSaleHistoryAllPages(fromDay: string, toDay: string, pageSize = 200) {
  ...
  const totalPages =
    isRecord(result) && typeof result["TotalPageCount"] === "number"
      ? Math.max(1, Math.floor(result["TotalPageCount"] as number))
      : 1;
...
export async function POST(request: Request) {
  try {
    ...
    const fromDay = today.toFormat("yyyy-LL-dd");
